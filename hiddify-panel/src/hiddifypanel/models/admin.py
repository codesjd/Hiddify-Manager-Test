...
class AdminUser(BaseAccount):
    ...
    # JSON list of Permission.value strings. Empty/None = "not restricted
    # beyond role" (see has_permission below) so every admin created before
    # this column existed keeps behaving exactly as before.
    permissions = Column(db.Text, nullable=True, default="[]")
    ...
    def has_permission(self, permission) -> bool:
        """super_admin always passes. For admin/agent: if this account has
        no explicit permissions configured (the default for every existing
        admin), fall back to True - i.e. behave exactly like before
        granular permissions existed. Only once a super_admin explicitly
        assigns a non-empty permission list to an account does it actually
        get restricted to that list."""
    ...
    def remove(self):
        ...
        DailyUsage.query.filter(DailyUsage.admin_id.in_(self.recursive_sub_admins_ids())).update(
