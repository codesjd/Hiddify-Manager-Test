...
from wtforms.validators import NumberRange, Optional, Regexp, ValidationError

from hiddifypanel import hutils
from hiddifypanel.auth import current_account, login_required
...
class DnsTT(BaseModel):
    ...
    mtu: int = Field(
        0,
        description="dnstt: maximum size of DNS responses (0-> default 1232). xdns: mKCP MTU for this domain's Xray inbound (0-> default 132, empirically confirmed via live testing) - Xray-core's xdns encoder hard-rejects any raw mKCP segment of 224 bytes or more (silently dropped, no retry) and real deployments hit that ceiling earlier than the theoretical 223-byte math suggests, so values above the confirmed-safe default risk breaking the tunnel completely; only raise this if you've verified higher values produce zero \"too long\" errors in your own client logs.",
    )

    keepalive: int = Field(
        0,
        description="keepalive ping interval in seconds; must be less than idle-timeout (0-> use default 2s)",
        ge=0,
        le=100,
    )
    idle_timeout: int = Field(
        0,
        description="session idle timeout in seconds; tears down sessions with no data within this period (0-> use default 10 seconds)",
        ge=0,
        le=100,
    )
    clientid_size: int = Field(
        0, description="client ID size in bytes (ignored when dnstt_compat is true) (0-> use default 2)"
    )
    dnstt_compat: bool = Field(False, description="use original dnstt wire format (8-byte ClientID, padding prefixes)")
    record_type: Literal["", "txt", "cname", "a", "aaaa", "mx", "ns", "srv"] = Field(
        "", description='DNS record type for downstream data (txt, cname, a, aaaa, mx, ns, srv) (""->default "txt")'
    )
    max_qname_len: int = Field(
        0, description="maximum total QNAME length in wire format (253 per RFC 1035) (0->default 101)"
    )
    open_stream_timeout: int = Field(
        0, description='timeout for opening an smux stream (e.g. 500ms, 3s) (0->default "10s")'
    )
    ...
    xdns_resolvers: str = Field(
        "", description='comma-separated "ip:port" resolvers for this xdns domain ("" -> use the server default)'
    )
...
class DomainAdmin(AdminLTEModelView):
    ...
    column_descriptions = dict(
        ...
        mode=_(
            "Direct mode means you want to use your server directly (for usual use), CDN means that you use your server on behind of a CDN provider. "
        ...
        also_enable_xicmp=_(
            "xICMP needs no DNS/NS setup, so this domain (including a bare server IP) can serve xICMP at the same time as its main mode. Creates/removes a matching xICMP entry for you - equivalent to adding it separately with the same domain value."
        ),
        ...
        http_port=_(
            "Plain-HTTP port for this domain. Leave empty to use the default port 80. A custom port is exclusive to this domain - no other domain can use it, and this domain won't be reachable on 80 anymore."
        ),
        tls_port=_(
            "TLS port for this domain. Leave empty to use the default port 443. A custom port is exclusive to this domain - no other domain can use it, and this domain won't be reachable on 443 anymore."
        ),
        reality_port=_(
            "REALITY direct port for this domain (special_reality_* modes only). Auto-generated the first time this domain is saved as a REALITY mode if left empty; edit it to set your own."
        ),
        reality_private_key=_(
            "REALITY private key for this domain. Auto-generated the first time this domain is saved as a REALITY mode if left empty; edit it to set your own."
        ),
        reality_public_key=_(
            "REALITY public key for this domain, matching the private key above. Auto-generated the first time this domain is saved as a REALITY mode if left empty; edit it to set your own."
        ),
        reality_short_id=_(
            "REALITY short ID for this domain. Auto-generated the first time this domain is saved as a REALITY mode if left empty; edit it to set your own."
        ),
        hysteria_port=_(
            "Hysteria2 port for this domain (direct/relay/fake modes only). Leave empty to keep the automatically assigned port; set a value to pin this domain to a specific one."
        ),
        tuic_port=_(
            "TUIC port for this domain (direct/relay/fake modes only). Leave empty to keep the automatically assigned port; set a value to pin this domain to a specific one."
        ),
        naive_port=_(
            "Naive port for this domain (direct/relay modes only). Leave empty to keep the automatically assigned port; set a value to pin this domain to a specific one."
        ),
        anytls_port=_(
            "AnyTLS port for this domain (direct/relay/fake modes only). Leave empty to keep the automatically assigned port; set a value to pin this domain to a specific one."
        ),
        dnstt_port=_(
            "DNSTT port for this domain (dnstt mode only). Leave empty to keep the automatically assigned port; set a value to pin this domain to a specific one."
        ),
        xdns_port=_(
            "xDNS port for this domain (xdns mode only). Leave empty to keep the automatically assigned port; set a value to pin this domain to a specific one."
        ),
        xicmp_port=_(
            "xICMP port for this domain (xicmp mode only). Leave empty to keep the automatically assigned port; set a value to pin this domain to a specific one."
        ),
        extra_params=_(
            "The dnstt-specific fields below are pre-filled with defaults. You can also add ANY other key here "
    ...
    form_widget_args = {
        ...
        "show_domains": {"class": "form-control ltr", "data-role": ""},
        "download_domain": {"class": "form-control ltr", "data-role": ""},
    }
    ...
    form_args = {
        ...
        "domain": {
            "validators": [
                Regexp(
                    r"^(\*\.)?([A-Za-z0-9\-\.]+\.[a-zA-Z]{2,})$|^$|^(\d{1,3}\.){3}\d{1,3}$|^([0-9a-fA-F]{1,4}:){1,7}(:|[0-9a-fA-F]{1,4})$",
        ...
        "http_port": {"validators": [Optional(), NumberRange(min=1, max=65535, message=__("Invalid port"))]},
        "tls_port": {"validators": [Optional(), NumberRange(min=1, max=65535, message=__("Invalid port"))]},
        "reality_port": {"validators": [Optional(), NumberRange(min=1, max=65535, message=__("Invalid port"))]},
        "hysteria_port": {"validators": [Optional(), NumberRange(min=1, max=65535, message=__("Invalid port"))]},
        "tuic_port": {"validators": [Optional(), NumberRange(min=1, max=65535, message=__("Invalid port"))]},
        "naive_port": {"validators": [Optional(), NumberRange(min=1, max=65535, message=__("Invalid port"))]},
        "anytls_port": {"validators": [Optional(), NumberRange(min=1, max=65535, message=__("Invalid port"))]},
        "dnstt_port": {"validators": [Optional(), NumberRange(min=1, max=65535, message=__("Invalid port"))]},
        "xdns_port": {"validators": [Optional(), NumberRange(min=1, max=65535, message=__("Invalid port"))]},
        "xicmp_port": {"validators": [Optional(), NumberRange(min=1, max=65535, message=__("Invalid port"))]},
    }
    ...
    def on_model_change(self, form, model, is_created):
        ...
        if is_created or not old_db_domain or old_db_domain.mode != model.mode:
            ...
            sa_inspect(model).attrs[attr].history.has_changes()
            for attr in (
                "http_port",
                "tls_port",
                "reality_port",
    ...
    def _validate_port_exclusivity(self, model):
        ...
        if model.http_port and model.http_port != 80:
            conflict = Domain.query.filter(
                Domain.http_port == model.http_port, Domain.id != model.id, Domain.child_id == model.child_id
            ).first()
            if conflict:
        ...
        if model.tls_port and model.tls_port != 443:
            conflict = Domain.query.filter(
                Domain.tls_port == model.tls_port, Domain.id != model.id, Domain.child_id == model.child_id
            ).first()
            if conflict:
        ...
        if model.reality_port:
            ...
            conflict = Domain.query.filter(
                Domain.reality_port == model.reality_port, Domain.id != model.id, Domain.child_id == model.child_id
            ).first()
            if conflict:
        ...
        for field_name, label in (
            ("hysteria_port", _("Hysteria2")),
            ("tuic_port", _("TUIC")),
            ("naive_port", _("Naive")),
            ("anytls_port", _("AnyTLS")),
            ("dnstt_port", _("DNSTT")),
            ("xdns_port", _("xDNS")),
            ("xicmp_port", _("xICMP")),
        ):
            ...
            if value:
                conflict = Domain.query.filter(
                    getattr(Domain, field_name) == value, Domain.id != model.id, Domain.child_id == model.child_id
                ).first()
                if conflict:
                    raise ValidationError(
                        f"{_('This')} {label} {_('port is already assigned to domain: ')}{conflict.domain}"
                    )
    ...
    def _update_cloudflare(self, model, ipv4_list, ipv6_list):
        if hconfig(ConfigEnum.cloudflare) and model.mode not in [DomainType.fake, DomainType.relay, DomainType.reality]:
            try:
                ...
                if ipv4_list:
                    hutils.network.cf_api.add_or_update_dns_record(
                        model.domain, str(ipv4_list[0]), "A", proxied=proxied
                    )
                if ipv6_list:
                    hutils.network.cf_api.add_or_update_dns_record(
                        model.domain, str(ipv6_list[0]), "AAAA", proxied=proxied
                    )
                return True
    ...
    def _validate_not_used_before(self, model, is_created):
        ...
        for c in configs:
            if (
                "domain" in c
                and c not in [ConfigEnum.decoy_domain, ConfigEnum.reality_fallback_domain]
                and c.category != "hidden"
            ):
                if model.domain == configs[c]:
                    raise ValidationError(_("You have used this domain in: ") + _(f"config.{c}.label"))

        for td in Domain.query.filter(
            Domain.mode.in_(
                [
                    DomainType.reality,
            ...
            if td.servernames and (model.domain in td.servernames.split(",")):
                raise ValidationError(
                    _("You have used this domain in: ") + _(f"config.reality_server_names.label") + td.domain
                )

        if (
            is_created
            and Domain.query.filter(Domain.domain == model.domain, Domain.child_id == model.child_id).count() > 1
        ):
        ...
        if not domain_ip_matches_server and model.mode in [DomainType.direct]:
            raise ValidationError(
                __(
                    ...
                    server_ip=server_ips_str,
                    domain_ip=dips_str,
                )
            )

        if domain_ip_matches_server and model.mode in [
            DomainType.cdn,
            DomainType.relay,
            DomainType.fake,
    ...
    def on_model_delete(self, model):
        ...
        if (
            hconfig(ConfigEnum.cloudflare)
            and model.mode not in [DomainType.fake, DomainType.reality, DomainType.relay]
            and "special" not in model.mode
        ):
    ...
    def is_accessible(self):
        if (
            login_required(roles={Role.super_admin, Role.admin}, permissions={Permission.manage_domains})(
                lambda: True
            )()
            != True
        ):
