...
from hiddifypanel.models import Child, Domain, DomainProxyOverride, Proxy, get_hconfigs
...
def _build_form(domain: Domain, empty=False):
    ...
    for proxy in proxies:
        ...
        field = SwitchField(
            proxy.name, default=current, description=f"l3:{proxy.l3} transport:{proxy.transport} cdn:{proxy.cdn}"
        )
        ...
        for field_name, submitted in form.data.items():
            ...
            if submitted == baseline:
                ...
                if existing:
                    ...
                    db.session.add(
                        DomainProxyOverride(domain_id=domain.id, proxy_id=proxy_id, enable=submitted, params={})
                    )
