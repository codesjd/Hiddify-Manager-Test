...
class UserView(FlaskView):
    ...
    def xray(self):
        ...
        all_configs = hutils.proxy.xrayjson.configs_as_json(
            c["domains"], c["user"], c["expire_days"], c["profile_title"]
        )
    ...
    def get_proper_config(self):
        ...
        if hconfig(ConfigEnum.sub_full_xray_json_enable):
            ...
            if g.user_agent.get("is_v2rayng") and hutils.flask.is_client_version(
        ...
        if re.match(
            "^(Hiddify|FoXray|Fair|v2rayNG|SagerNet|Shadowrocket|V2Box|Loon|Liberty|Streisand)", ua, re.IGNORECASE
        ):
            return self.links_imp(base64=True)
    ...
    def singbox_ssh_imp(self):
        ...
        if request.method == "HEAD":
            ...
            resp = render_template(
                "singbox_config.json",
                **c,
                host_keys=hutils.proxy.get_ssh_hostkeys(get_hconfigs(), True),
                ssh_client_version=hiddify.get_ssh_client_version(user),
...
def get_common_data(user_uuid, mode, no_domain=False, filter_domain=None, skip_ip_debug=False):
    ...
    expire_s = int(
        (datetime.date.today() + datetime.timedelta(days=expire_days) - datetime.date(1970, 1, 1)).total_seconds()
    )
...
def add_headers(res, c, mimetype="text/plain"):
    ...
    resp.headers["Subscription-Userinfo"] = (
        f"upload=0;download={c['usage_current_b']};total={c['usage_limit_b']};expire={c['expire_s']}"
    )
