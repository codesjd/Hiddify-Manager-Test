...
def commander(
    command: Command, run_in_background=True, subsystems: set[str] | frozenset[str] | None = None, **kwargs: str | int
) -> str | None:
    """
    ...
    """
    base_cmd: List[str] = ["sudo", os.path.join(os.environ["HIDDIFY_CONFIG_PATH"], "common/commander.py")]
...
def cmd_in_back(cmd, on_success=None):
    ...
    if p.returncode != 0:
        ...
        logging.getLogger(__name__).error(f"Background command failed with exit code {p.returncode}: {' '.join(cmd)}")
    elif on_success:
        on_success()
