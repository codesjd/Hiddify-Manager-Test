...
class InfoAPI(MethodView):
    ...
    def get(self):
        ...
        dto.brand_icon_url = (
            "" if hconfig(ConfigEnum.branding_title) else hutils.flask.static_url_for(filename="images/favicon.ico")
        )
        ...
        dto.admin_message_html = hutils.encode.sanitize_html(hconfig(ConfigEnum.branding_freetext)) or _(
        ...
        if not hconfig(ConfigEnum.branding_freetext) and auth.admin_session_is_exist():
            dto.admin_message_html += (
                "<p style='font-style: italic;font-size:8px'>"
                + _("[Admin only visible message:] You can change this message from settings")
                + "</p>"
            )
