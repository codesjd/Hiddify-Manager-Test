...
def admin_keyboard_main():
    ...
    return types.InlineKeyboardMarkup(
        keyboard=[[types.InlineKeyboardButton(text=_("Create Package"), callback_data=f"create_package")]]
    )
...
def admin_keyboard_days(old_action):
    ...
    return types.InlineKeyboardMarkup(
        keyboard=[[keyboard(i) for i in range(1, 16, 3)], [keyboard(30 * i) for i in range(1, 5)]]
    )
...
def admin_keyboard_domain(old_action):
    def keyboard(domain):
        return types.InlineKeyboardButton(
            text=f"{domain.alias or domain.domain}", callback_data=f"{old_action} {domain.id}"
        )
        ...
        try:
            ...
            if len(splt) == 1:
                ...
                bot.edit_message_text(
                    new_text, call.message.chat.id, call.message.message_id, reply_markup=admin_keyboard_gig(call.data)
                )
                bot.answer_callback_query(call.id, text=_("Ok"), show_alert=False, cache_time=1)
            elif len(splt) == 2:
                new_text = _("package days?")
                bot.edit_message_text(
                    new_text, call.message.chat.id, call.message.message_id, reply_markup=admin_keyboard_days(call.data)
                )
                bot.answer_callback_query(call.id, text=_("Ok"), show_alert=False, cache_time=1)
            elif len(splt) == 3:
                new_text = _("How many?")
                bot.edit_message_text(
                    ...
                    reply_markup=admin_keyboard_count(call.data),
                )
                bot.answer_callback_query(call.id, text=_("Ok"), show_alert=False, cache_time=1)
            elif len(splt) == 4:
                new_text = _("Domain?")
                bot.edit_message_text(
                    ...
                    call.message.chat.id,
                    call.message.message_id,
                    reply_markup=admin_keyboard_domain(call.data),
                )
                bot.answer_callback_query(call.id, text=_("Ok"), show_alert=False, cache_time=1)
            elif len(splt) == 5:
                gig = int(splt[1])
                ...
                for i in range(1, count + 1):
                    ...
                    user = User(
                        package_days=days,
                        usage_limit_GB=gig,
                        name=f"{admin_name} auto {i}  {datetime.date.today()}",
                        added_by=admin_id,
                    )
                    db.session.add(user)
                    db.session.commit()
                    ...
                    with force_locale(user.lang or hconfig(ConfigEnum.lang)):
                        bot.send_message(
                            call.message.chat.id,
                            Usage.get_usage_msg(user.uuid, domain),
                            reply_markup=Usage.user_keyboard(user.uuid),
                        )

                # db.session.commit()
                new_text = _("Finished")
                bot.edit_message_text(
                    new_text, call.message.chat.id, call.message.message_id, reply_markup=admin_keyboard_main()
                )
                bot.answer_callback_query(call.id, text=_("Ok"), show_alert=False, cache_time=1)
