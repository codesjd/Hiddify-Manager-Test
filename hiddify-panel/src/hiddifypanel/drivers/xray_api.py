...
class XrayApi(DriverABS):
    ...
    def __add_uuid_to_tag(self, uuid, t):
        ...
        xray_client.add_client(
            t, f"{uuid}", f"{uuid}@hiddify.com", protocol=protocol, flow=flow, alter_id=0, cipher="chacha20_poly1305"
        )
    ...
    def get_enabled_users_terminal(self):
        ...
        for t in tags:
            ...
            try:
                ...
                users = [
                    splt[0]
                    for splt in [u.get("email", "").split("@") for u in data.get("users", [])]
                    if len(splt) == 2 and splt[1] == "hiddify.com"
                ]
                if len(users) > 0:
                    return users
