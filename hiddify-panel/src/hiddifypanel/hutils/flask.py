...
def __parse_user_agent(ua: str) -> dict:
    ...
    res["is_clash_meta"] = (
        re.match("^(Clash-verge|Clash-?Meta|Stash|NekoBox|NekoRay|Pharos|hiddify-desktop)", ua, re.IGNORECASE) and True
    )
    ...
    res["is_v2ray"] = (
        re.match("^(Hiddify|FoXray|Fair|v2rayNG|SagerNet|Shadowrocket|V2Box|Loon|Liberty)", ua, re.IGNORECASE) and True
    )
...
def is_admin_proxy_path() -> bool:
    ...
    return proxy_path in [hconfig(ConfigEnum.proxy_path_admin)] or (
        proxy_path in [hconfig(ConfigEnum.proxy_path)] and "/admin/" in request.path
    )


def is_client_proxy_path() -> bool:
    proxy_path = g.get("proxy_path") or get_proxy_path_from_url(request.url)
    return proxy_path in [hconfig(ConfigEnum.proxy_path_client)] or (
        proxy_path in [hconfig(ConfigEnum.proxy_path)] and "/admin/" not in request.path
    )
...
def proxy_path_validator(proxy_path: str) -> None:
    ...
    if is_api_call(request.path):
        if __is_admin_api_call() and proxy_path != admin_proxy_path:
            return (
                flask_abort(400, Markup(f"Invalid Proxy Path <a href=/{admin_proxy_path}/admin>Admin Panel</a>"))
                if dbg_mode
                else apiflask_abort(400, "invalid request")
            )
        if is_user_api_call() and proxy_path != client_proxy_path:
            return (
                flask_abort(400, Markup(f"Invalid Proxy Path <a href=/{client_proxy_path}/admin>User Panel</a>"))
                if dbg_mode
                else apiflask_abort(400, "invalid request")
            )
