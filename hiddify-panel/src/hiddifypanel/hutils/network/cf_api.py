...
def add_or_update_dns_record(domain: str, ip: str, dns_type: str = "A", proxied: bool = True) -> bool:
    ...
    if zone:
        ...
        if not record:
            api_res = __cf.dns.records.create(
                zone_id=zone.id, name=dns_name, type=dns_type, content=ip, proxied=proxied
            )
        else:
            api_res = __cf.dns.records.edit(
                zone_id=zone.id, dns_record_id=record.id, name=dns_name, type=dns_type, content=ip, proxied=proxied
            )
