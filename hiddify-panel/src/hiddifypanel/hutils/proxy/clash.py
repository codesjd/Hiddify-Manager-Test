...
def to_clash(proxy, meta_or_normal):
    ...
    if (
        proxy["l3"] == ProxyL3.tls_h2
        and proxy["proto"] in [ProxyProto.vmess, ProxyProto.vless]
        and proxy["dbe"].cdn == ProxyCDN.direct
    ):
    ...
    if proxy["proto"] == "ssr":
        ...
        if proxy["transport"] == "faketls":
            base["plugin"] = "obfs"
            base["plugin-opts"] = {"mode": "tls", "host": proxy["fakedomain"]}
    ...
    if base["network"] == "tcp" and proxy["alpn"] != "h2":
        ...
        if "path" in proxy:
            base["http-opts"] = {"path": [proxy["path"]]}
            if "host" in proxy:
    ...
    if base["network"] == "tcp" and proxy["alpn"] == "h2":
        ...
        if "path" in proxy:
            base["h2-opts"] = {"path": proxy["path"]}
            if "host" in proxy:
                base["h2-opts"]["host"] = [proxy["host"]]
    if base["network"] == "grpc":
        base["grpc-opts"] = {"grpc-service-name": proxy["grpc_service_name"]}
