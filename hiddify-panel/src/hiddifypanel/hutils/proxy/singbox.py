...
def configs_as_json(domains: list[Domain], **kwargs) -> dict:
    ...
    for pinfo in hutils.proxy.get_valid_proxies(domains):
        ...
        if "msg" not in sing:
            if (
                hutils.flask.is_client_version(hutils.flask.ClientVersion.hiddify_next, 4, 0, 0)
                and sing[0]["type"] == "wireguard"
            ):
                endpoints += sing
            else:
                allp += sing
...
def add_multiplex(base: dict, proxy: dict):
    if proxy.get("mux_enable") != "singbox":
        return
    base["multiplex"] = {"enabled": True, "protocol": proxy["mux_protocol"], "padding": proxy["mux_padding_enable"]}
...
def add_udp_over_tcp(base: dict):
    base["udp_over_tcp"] = {"enabled": True, "version": 2}
...
def add_tls(base: dict, proxy: dict):
    ...
    if proxy["proto"] not in ["tuic", "hysteria2"] and proxy["transport"] != "xhttp":
        base["tls"]["utls"] = {"enabled": True, "fingerprint": proxy.get("fingerprint", "none")}
...
def add_transport(base: dict, proxy: dict):
    ...
    if proxy["transport"] in ["ws", "WS"]:
        base["transport"] = {"type": "ws", "path": proxy["path"], "early_data_header_name": "Sec-WebSocket-Protocol"}
        if "host" in proxy:
    ...
    if proxy["transport"] in [ProxyTransport.httpupgrade]:
        base["transport"] = {"type": "httpupgrade", "path": proxy["path"]}
        if "host" in proxy:
            base["transport"]["headers"] = {"Host": proxy["host"]}
...
def add_naive(base: dict, proxy: dict):
    ...
    base["extra_headers"] = {"hiddify-naive-secret": proxy["path"]}
    ...
    for port in proxy["tcp_ports"]:
        if port:
            base["portBindings"].append(
                {"protocol": "TCP", "port": 0 if "-" in port else int(port), "portRange": port if "-" in port else ""}
            )
    for port in proxy["udp_ports"]:
        if port:
            base["portBindings"].append(
                {"protocol": "UDP", "port": 0 if "-" in port else int(port), "portRange": port if "-" in port else ""}
            )
...
def add_wireguard(base: dict, proxy: dict):
    if hutils.flask.is_client_version(hutils.flask.ClientVersion.singbox, 1, 13, 0):
        ...
        del base["server"]
        if g.user_agent.get("is_hiddify"):
            base["noise"] = {"fake_packet": {"enabled": True, "count": proxy["wg_noise_trick"]}}
        ...
        base["mtu"] = 1380
        if g.user_agent.get("is_hiddify") and hutils.flask.is_client_version(
            hutils.flask.ClientVersion.hiddify_next, 0, 15, 0
        ):
    ...
    if proxy["transport"] == "shadowtls":
        ...
        shadowtls_base = {
            ...
            "tls": {
                ...
                "utls": {"enabled": True, "fingerprint": proxy.get("fingerprint", "none")},
...
def add_hysteria(base: dict, proxy: dict):
    ...
    if proxy.get("hysteria_obfs_enable"):
        base["obfs"] = {"type": "salamander", "password": proxy.get("hysteria_obfs_password")}
    base["password"] = proxy["uuid"]
