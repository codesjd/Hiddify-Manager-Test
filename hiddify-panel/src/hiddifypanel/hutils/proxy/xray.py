...
def to_link(proxy: dict) -> str | dict:
    ...
    if proxy["proto"] in ["ss", "v2ray"]:
        ...
        if proxy["transport"] == "faketls":
            return (
                f'{baseurl}?plugin=obfs-local&obfs-host={proxy["fakedomain"]}&obfs=http&udp-over-tcp=true#{name_link}'
            )
...
def make_v2ray_configs(domains: list[Domain], user: User, expire_days: int, ip_debug=None) -> str:
    ...
    if hconfig(ConfigEnum.show_usage_in_sublink) and not g.user_agent.get("is_hiddify"):
        ...
        if user.usage_limit_GB < 1000:
            ...
            name += (
                f'{round(user.current_usage_GB/1000, 3)}/{str(round(user.usage_limit_GB/1000, 1)).replace(".0", "")}TB'
            )
        else:
        ...
        if len(name) > 3:
            res.append(
                f"trojan://1@{fake_ip_for_sub_link}?sni=fake_ip_for_sub_link&security=tls#{hutils.encode.url_encode(name)}"
            )
...
def add_tls_tricks_to_dict(d: dict, proxy: dict):
    if proxy.get("tls_fragment_enable"):
        ...
        d["fragment"] = (
            f'{proxy["tls_fragment_size"]},{proxy["tls_fragment_sleep"]},{proxy.get("tls_fragment_packets", "tlshello")}'
        )
        # if g.user_agent.get('is_streisand'):
        # else:
