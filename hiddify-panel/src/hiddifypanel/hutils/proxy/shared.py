...
def is_proxy_valid(proxy: Proxy, domain_db: Domain, port: int) -> dict | None:
    ...
    if proxy.proto != ProxyProto.dnstt and domain_db.mode == DomainType.dnstt:
        return {
            "name": name,
            "msg": "dnstt domain only works with dnstt protocol",
            "type": "error",
            "proto": proxy.proto,
        }
    ...
    if proxy.proto != ProxyProto.xicmp and domain_db.mode == DomainType.xicmp:
        return {
            "name": name,
            "msg": "xicmp domain only works with xicmp protocol",
            "type": "error",
            "proto": proxy.proto,
        }
    ...
    if proxy.proto in (ProxyProto.xdns, ProxyProto.xicmp) and domain_db.mode not in (DomainType.xdns, DomainType.xicmp):
        return {
            "name": name,
            "msg": "xdns/xicmp protocol only works with a matching-mode domain",
            "type": "debug",
            "proto": proxy.proto,
        }
    ...
    for p in ["tcp", "grpc", "xhttp"]:
        if "reality" in l3 and p in domain_db.mode and p not in proxy.transport:
            return {
                "name": name,
                "msg": f"reality proxy {proxy.transport} in reality {p} domain",
                "type": "debug",
                "proto": proxy.proto,
            }
        if "reality" in l3 and p not in domain_db.mode and p in proxy.transport:
            return {
                "name": name,
                "msg": f"reality {p} proxy not in reality {p} domain",
                "type": "debug",
                "proto": proxy.proto,
            }
...
def get_proxies(child_id: int = 0, only_enabled=False) -> list["Proxy"]:
    ...
    if not Domain.query.filter(
        Domain.mode.in_([DomainType.cdn, DomainType.auto_cdn_ip]),
        Domain.servernames != "",
        Domain.servernames != Domain.domain,
    ).first():
...
def _get_valid_proxies_uncached(domains: list[Domain]) -> list[dict]:
    ...
    if domains_needing_dns:
        with ThreadPoolExecutor(max_workers=min(len(domains_needing_dns), 20)) as executor:
            future_to_domain = {
                executor.submit(hutils.network.get_domain_ips_cached, d): d for d in domains_needing_dns
            }
    ...
    if any(d.servernames for d in domains):
        ech_enabled_children = {
            cid for cid in {d.child_id for d in domains} if get_hconfigs(cid).get(ConfigEnum.tls_ech_enable)
        }
        for d in domains:
    ...
    for domain in domains:
        ...
        if domain_overrides:
            ...
            if missing_ids:
                if domain.child_id not in all_proxies_by_id_map:
                    all_proxies_by_id_map[domain.child_id] = {
                        p.id: p for p in Proxy.query.filter_by(child_id=domain.child_id).all()
                    }
        ...
        for proxy in domain_proxies:
            ...
            if proxy.proto in [ProxyProto.ss] and proxy.transport not in [
                ProxyTransport.grpc,
                ProxyTransport.h2,
                ProxyTransport.WS,
                ProxyTransport.httpupgrade,
                ProxyTransport.xhttp,
            ]:
                noDomainProxies = True
            options = []
...
def apply_proxy_overrides(pinfo: dict, proxy: Proxy) -> None:
    """Apply this Proxy row's `params` JSON (editable from the new
    'Inbound Overrides' admin page) on top of the generated proxy dict, in
    place. `params` already existed on the Proxy model and was already
    being stashed into the generated dict as a nested, inert sub-object -
    this actually applies it as an override, same pattern as
    apply_domain_overrides but scoped to "every domain using this
    protocol/transport/cdn combination" instead of one specific domain.
    Domain-level overrides are applied after this and win on conflicts.
    """
    if not proxy.params:
        return
    for key, value in proxy.params.items():
        if key in _OVERRIDE_BLOCKLIST:
            continue
        pinfo[key] = value


# Keys that must never be touched by a per-domain override - identity/account
# data and internal bookkeeping objects. Everything else in the generated
# proxy dict (sni, host, security-related fields, obfs/mux params, alpn,
...
def apply_domain_overrides(pinfo: dict, domain_db: Domain) -> None:
    """Apply this domain's per-domain override JSON (Domain.extra_params,
    edited in the admin panel's domain form) on top of the auto-generated
    proxy dict, in place.

    This is the same `extra_params` field/mechanism that already existed for
    dnstt only (see the dnstt branch in make_proxy()); it's now applied for
    every protocol so a single domain (e.g. one relay domain) can carry its
    own transport/security tweaks (say, a different `fingerprint`,
    `hysteria_obfs_password`, or `alpn`) without that also changing every
    other domain of the same mode.

    Silently ignores keys in _OVERRIDE_BLOCKLIST and any error parsing the
    JSON (an admin typo here should never break subscription generation for
    everyone).
    """
    try:
        overrides = domain_db.extra_params_json()
    except Exception:
        return
    if not overrides:
        return
...
def make_proxy(hconfigs: dict, proxy: Proxy, domain_db: Domain, phttp=80, ptls=443, pport: int | None = None) -> dict:
    ...
    if base["proto"] in {ProxyProto.mieru}:
        ...
        base["tcp_ports"] = (
            ports_to_ranges(hconfigs.get(ConfigEnum.mieru_tcp_ports)) if proxy.transport == ProxyTransport.tcp else []
        )
        base["udp_ports"] = (
            ports_to_ranges(hconfigs.get(ConfigEnum.mieru_udp_ports)) if proxy.transport == ProxyTransport.udp else []
        )
    ...
    if base["proto"] in ["v2ray", "ss"]:
        ...
        base["password"] = (
            f'{hutils.encode.do_base_64(hconfigs[ConfigEnum.shared_secret].replace("-",""))}:{hutils.encode.do_base_64(g.account.uuid.replace("-",""))}'
        )
